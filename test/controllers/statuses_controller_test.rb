...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference('Status.count') do
      post statuses_url, params: { status: { status: @status.status, status_date: @status.status_date } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { status: @status.status, status_date: @status.status_date } }
